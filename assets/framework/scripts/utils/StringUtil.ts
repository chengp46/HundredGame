...
export class StringUtil {
     ...
     * @param str 分隔符
     */
    static stringToArray(str: string, divide:string = ',') {
        if (str == "") {
            return [];
        }
     ...
     */
    static sub(str: string, maxLength: number, showdot: boolean = false) {
        let chars = Array.from(str); // 处理 UTF-16/Emoji
        return chars.length > maxLength ? chars.slice(0, maxLength).join("") + showdot? "...": '' : str;
    }
